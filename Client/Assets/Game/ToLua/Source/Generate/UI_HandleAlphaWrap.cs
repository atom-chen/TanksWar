...
public class UI_HandleAlphaWrap
{
	public static void Register(LuaState L)
	{
		L.BeginClass(typeof(UI.HandleAlpha), typeof(UI.IHandle));
		...
		L.RegFunction("New", _CreateUI_HandleAlpha);
	...
	static int _CreateUI_HandleAlpha(IntPtr L)
	{
		try
		{
			...
			if (count == 0)
			{
				UI.HandleAlpha obj = new UI.HandleAlpha();
				ToLua.PushObject(L, obj);
				return 1;
			}
			else
			{
				return LuaDLL.luaL_throw(L, "invalid arguments to ctor method: UI.HandleAlpha.New");
			}
		}
		catch(Exception e)
		{
	...
	static int OnUseNowStart(IntPtr L)
	{
		try
		{
			...
			UI.HandleAlpha obj = (UI.HandleAlpha)ToLua.CheckObject(L, 1, typeof(UI.HandleAlpha));
	...
	static int get_IsDurationValid(IntPtr L)
	{
		...
		try
		{
			...
			UI.HandleAlpha obj = (UI.HandleAlpha)o;
