...
public class UI_HandleSizeWrap
{
	public static void Register(LuaState L)
	{
		L.BeginClass(typeof(UI.HandleSize), typeof(UI.IHandle));
		...
		L.RegFunction("New", _CreateUI_HandleSize);
	...
	static int _CreateUI_HandleSize(IntPtr L)
	{
		try
		{
			...
			if (count == 0)
			{
				UI.HandleSize obj = new UI.HandleSize();
				ToLua.PushObject(L, obj);
				return 1;
			}
			else
			{
				return LuaDLL.luaL_throw(L, "invalid arguments to ctor method: UI.HandleSize.New");
			}
		}
		catch(Exception e)
		{
	...
	static int OnUseNowStart(IntPtr L)
	{
		try
		{
			...
			UI.HandleSize obj = (UI.HandleSize)ToLua.CheckObject(L, 1, typeof(UI.HandleSize));
	...
	static int get_IsDurationValid(IntPtr L)
	{
		...
		try
		{
			...
			UI.HandleSize obj = (UI.HandleSize)o;
