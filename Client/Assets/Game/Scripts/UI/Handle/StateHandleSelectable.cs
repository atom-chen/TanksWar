...
namespace UI
{
    ...
    //ICancelHandler
    {
        ...
        Action<LuaTable> m_onClickLua;
        Action<LuaTable, StateHandle> m_onClickExLua;
        ...
        public void AddLuaClick(Action<LuaTable> cb, LuaTable tbl)
        {
            ...
            //如果重复添加，那么就不添加了
            Delegate[] inlist = m_onClickLua.GetInvocationList();
            foreach (Delegate d in inlist)
            {
                if (d == cb)
                    return;
        ...
        public void AddLuaClickEx(Action<LuaTable, StateHandle> cb, LuaTable tbl)
        {
            ...
            //如果重复添加，那么就不添加了
            Delegate[] inlist = m_onClickExLua.GetInvocationList();
            foreach (Delegate d in inlist)
            {
                if (d == cb)
                    return;
        ...
        public void OnPointerClick(PointerEventData eventData)
        {
            ...
            if (m_onClickExLua != null)
                m_onClickExLua(m_luaTbl, this);

            if (m_onClick != null)
                m_onClick();
                   
            if (m_onClickLua != null)
                m_onClickLua(m_luaTbl);
