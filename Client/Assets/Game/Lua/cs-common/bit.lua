...
function __andBit(left,right)    --与  
    return (left == 1 and right == 1) and 1 or 0  
end  
  
function __orBit(left, right)    --或  
    return (left == 1 or right == 1) and 1 or 0  
end  
  
function __xorBit(left, right)   --异或  
    return (left + right) == 1 and 1 or 0  
end  
  
function __base(left, right, op) --对每一位进行op运算，然后将值返回  
    if left < right then  
...
function Bit.andOp(left, right)  
    return __base(left, right, __andBit)  
end  
  
function Bit.xorOp(left, right)  
    return __base(left, right, __xorBit)  
end  
  
function Bit.orOp(left, right)  
    return __base(left, right, __orBit)  
end  
  
function Bit.notOp(left)  
    return left > 0 and -(left + 1) or -left - 1  
end  
  
function Bit.lShiftOp(left, num)  --left左移num位  
    return left * (2 ^ num)  
end  
  
function Bit.rShiftOp(left,num)  --right右移num位  
